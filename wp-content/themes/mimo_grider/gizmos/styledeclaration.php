...
class GantryGizmoStyleDeclaration extends GantryGizmo {
	...
	function query_parsed_init() {
        ...
        $css = 'a, .separator ,.tweet_date {color:'.$gantry->get('linkcolor').';}';
        
$css .= '.fraseblock, .mediumfraseblock, .smallfraseblock, .tagcloud a:hover, .module-content-tags a:hover, #theaccordion ul li a:hover {background-color:'.$gantry->get('linkcolor').';}';

$css .= '.greysmallfraseblock, .greymediumfraseblock, .greyfraseblock, .readon_blog, .readon_recent, .flex-direction-nav a:hover, .prev-page a:hover, .next-page a:hover, .flickr-wall .page-number, .pagination a:hover, .tagcloud a:hover, .rt-tags a:hover, .rt-tags a:hover, .criteria_total {background-color:'.$gantry->get('linkcolor').';}';
	  //Frases Colors
	    $css .= '.standardpost, .fcfraseblock, .fcmediumfraseblock, .fcsmallfraseblock{background-color:'.$gantry->get('firstbg').';}';
		$css .= '.videopost, .scfraseblock, .scmediumfraseblock, .scsmallfraseblock{background-color:'.$gantry->get('secondbg').';}';
		$css .= '.audiopost, .tcfraseblock, .tcmediumfraseblock, .tcsmallfraseblock {background-color:'.$gantry->get('thirdbg').';}';
		$css .= '.linkpost, .focfraseblock, .focmediumfraseblock, .focsmallfraseblock {background-color:'.$gantry->get('fourthbg').';}';
		$css .= '.imagepost, .ficfraseblock, .ficmediumfraseblock, .ficsmallfraseblock {background-color:'.$gantry->get('fidthbg').';}';
		//Logo Image
		$css  .= '.logo-block {background:'.$gantry->get('logobg').';}';
		$css  .= '.gf-menu li:hover {background:'.$gantry->get('linkcolor').';}';
		
		//Body
		$css  .= 'body {background:url('.get_site_url().'/wp-content/themes/mimo_grider/images/backgrounds/'.$gantry->get('bgimage').') 0 0 repeat fixed '.$gantry->get('bodybg').';color:'.$gantry->get('bodytext').';} #rt-header, .frase, .mediumfrase, .smallfrase{color:'.$gantry->get('bodytext').';}';
		//Container
		$css  .= '.rt-container {background:'.$gantry->get('containerbg').';}';
        // Gradients
        $css .= '.button, button.validate, #member-profile a, #member-registration a, .formelm-buttons button, .btn-primary {background-color: '.$linkColor->lighten('4%').'; '.$this->_createGradient('top', $linkColor->lighten('4%'), '1', '0%', $linkColor->darken('9%'), '1', '100%').'}'."\n";
        $css .= '.button:hover, button.validate:hover, #member-profile a:hover, #member-registration a:hover, .formelm-buttons button:hover, .btn-primary:hover {background-color: '.$linkColor->lighten('10%').'; '.$this->_createGradient('top', $linkColor->lighten('10%'), '1', '0%', $linkColor->darken('3%'), '1', '100%').'}'."\n";
        $css .= '.button:active, button.validate:active, #member-profile a:active, #member-registration a:active, .formelm-buttons button:active, .btn-primary:active {background-color: '.$linkColor->darken('2%').'; '.$this->_createGradient('top', $linkColor->darken('2%'), '1', '0%', $linkColor->lighten('8%'), '1', '100%').'}'."\n";
