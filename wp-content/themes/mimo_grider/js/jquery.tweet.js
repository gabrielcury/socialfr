...
(function (factory) {
  ...
  jQuery.fn.tweet = function(o){
    ...
    function extract_template_data(item){
      ...
      o.retweeted_tweet_text = jQuery([linkURLs(item.text, o.entities)]).linkUser().linkHash()[0];
    ...
    function render_tweets(widget, tweets) {
      ...
      list.append(jQuery.map(tweets, function(o) { return '<li class="twitter_li"><div>'+ o.user +'</div><div class="twitter_text">'+ t(s.template, o) + '</div><div class="clear"></div></li>'; }).join('')).
    ...
    function load(widget) {
      ...
      if (s.loading_text) jQuery(widget).not(":has(.tweet_list)").empty().append(loading);
      jQuery.getJSON(build_api_url(), function(data){
        var tweets = jQuery.map(data.results || data, extract_template_data);
        tweets = jQuery.grep(tweets, s.filter).sort(s.comparator).slice(0, s.count);
        jQuery(widget).trigger("tweet:retrieved", [tweets]);
      });
    }
    ...
    return this.each(function(i, widget){
      ...
      jQuery(widget).unbind("tweet:render").unbind("tweet:retrieved").unbind("tweet:load").
        bind({
          "tweet:load": function() { load(widget); },
          "tweet:retrieved": function(ev, tweets) {
            jQuery(widget).trigger("tweet:render", [tweets])
          },
          "tweet:render": function(ev, tweets) {
            render_tweets(jQuery(widget), tweets);
          }
        }).trigger("tweet:load");
    });
  };
